...
{ // Uma constante tipo char recebe um str que é um ponteiro
    for (int i = 0; str[i] != '\0'; i++)
    { // Laco verifica um por um até chegar no final que seria o '\0' valor nulo
        if (!isdigit(str[i]))
            return 0; // Se algum caractere não for um dígito numérico, a função retorna 0 (falso), pois a string contém algo além de números
    }
...
int ehFloat(const char *str)
{
    int ponto = 0;
    if (str[0] == '\0')
        return 0; // Proibi enter sem nada
    for (int i = 0; str[i] != '\0'; i++)
    { // Vai um por um até chegar no final que seria o '\0' valor nulo
        if (str[i] == '.')
        ...
        { // Se algum caractere nao for digito retona 0(falso) , isdigit verifica se e numero , como tem ! verifica se nao tem numero
            return 0;
        }
    }
    return 1;
...
{
    for (int i = 0; str[i] != '\0'; i++)
    { // Vai um por um ate chegar no valor final '\0'
        if (!isalpha(str[i]) && str[i] != ' ')
        { /// isalpha verifica se é letra e verifica se e espaco
            return 0;
        }
    }
    return 1;
